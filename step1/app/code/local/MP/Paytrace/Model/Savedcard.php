 ...
 *
 */
class MP_Paytrace_Model_Savedcard extends Mage_Paygate_Model_Authorizenet
{
    /**
     * 
     * @var unknown
     ...
     * @var unknown
     */
    const METHOD_CODE = 'mp_paytrace_saved';
    
    /**
     * 
     * @var unknown
     */
    protected $_code  = 'mp_paytrace_saved';
    
    /**
     * Form block type
     */
    protected $_formBlockType = 'mp_paytrace/revolution_cc';
...
//        $client->setMethod(Zend_Http_Client::POST);
        ...
        $paramlist = Mage::helper('mp_paytrace/paytrace')->_createParmList(
                array_keys($request->getData()), 
                $request->getData()
                );

        try {
            $responseBody = Mage::helper('mp_paytrace/paytrace')->_call($paramlist);
        } catch (Exception $e) {
...
//                ->setBalanceOnCard($responseBody[54])
             ...
             Mage::throwException(
                Mage::helper('mp_paytrace')->__('Error in payment gateway.')
            );
        }
